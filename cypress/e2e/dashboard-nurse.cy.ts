const Head = ['No', 'Nomor Antrian', 'Poli/ Ruangan', 'Tanggal Masuk', 'No. eRM', 'NIK', 'Nama Pasien', 'Jenis Kelamin', 'Tempat & Tanggal Lahir', 'Asuransi', 'TTV'];
// const Data = [
...
function validateTableHeaders() {
  cy.get('[data-testid=table] thead th').each(($th, index) => {
    expect($th).to.contain( Head[index]);
  });
}
...
describe('Table Component Test', () => {
  beforeEach(() => {
    ...
    cy.visit('http://localhost:3000/nurse-dashboard');
  });
